...
def remove_think_blocks(text: str):
    """
    Remove all <think>...</think> blocks (multiple + multiline),
    remove any empty/whitespace-only lines after removal,
    and return both the cleaned text and the number of think-blocks removed.
    """
...
def calculate_additional_reward(think_count: int):
    # TODO: adjust the reward calculation logic as needed
    """
    Calculate additional reward based on the number of think-blocks removed.
    Each think-block contributes a fixed negative reward.
    """
    reward_per_think = 0.1  # Define the penalty for each think-block
    total_reward = think_count * reward_per_think
    return total_reward
...
def compute_score(completion, test_cases, task=None, continuous=False):
    ...
    # print(f"Completion before removing think blocks: {completion}")
    completion, think_count = remove_think_blocks(completion)
    # print(f"Completion after removing think blocks: {completion}")
    # raise RuntimeError("Debugging remove_think_blocks")
